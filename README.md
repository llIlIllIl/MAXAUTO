...
1. OCR 로딩이 뜨면 시작 버튼을 누릅니다. (최초 시작시 OCR엔진 등을 다운받는데 시간이 소요될 수 있습니다.)
2. DJMAX Respect V 플레이 이후 결과창에서 오른쪽 상단에 박스가 표시될때까지 기다립니다. 박스가 보이지 않는다면 설정 과정을 다시 하거나, 시작 버튼을 눌렀는지 확인해주세요.
3. 박스가 표시되는 중에 아래 키에 따라 작업을 할 수 있습니다.
   3-1. DEL 키를 입력할 경우 등록 작업을 취소합니다.
   3-2. 만약 인식 오류가 있는 경우, Insert키를 입력하여 재인식이 가능합니다.
   3-3. "=" 버튼을 눌러, 점수 등을 수동으로 입력이 가능합니다. (*안정되지 않은 기능이며, OCR 프로그램의 작동을 멈출 수 있습니다.)
또는 지정 시간동안(기본 10초) 입력이 없을 경우, 자동으로 스코어를 등록합니다.
