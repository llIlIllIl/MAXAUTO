...
#define MMAPSIZE GameOverlayIPC::OverlayMapBytes

std::shared_ptr<spdlog::logger> Monitor::monitorLogger = spdlog::stderr_logger_mt ("monitorLogger");

namespace
{
    void WriteHiddenOverlayFrame (char *buf)
    {
        ZeroMemory ((PVOID)buf, MMAPSIZE);
        auto *header = reinterpret_cast<GameOverlayIPC::OverlayFrameHeader *> (buf);
        ...
        header->height = 1u;
        header->stride = 4u;
        header->format = GameOverlayIPC::OverlayFrameFormatRGBA;
        header->seq = 0u;
        header->payloadSize = 4u;
    }
}
...
int Monitor::CreateFileMap ()
{
    ...
    if (buf == NULL)
    {
        Monitor::monitorLogger->error ("failed to initialize mapped file Error {}", GetLastError ());
        return GENERAL_ERROR;
    }
    WriteHiddenOverlayFrame (buf);
    UnmapViewOfFile (buf);
    return STATUS_OK;
}
...
int Monitor::SendMessageToOverlay (char *message)
{
    ...
    ZeroMemory ((PVOID)buf, sizeof (GameOverlayIPC::OverlayFrameHeader));
    const auto messageBytes = min (
        strlen (message) + 1, static_cast<size_t> (GameOverlayIPC::LegacyTextMapBytes));
    CopyMemory ((PVOID)buf, message, messageBytes * sizeof (char));
    buf[GameOverlayIPC::LegacyTextMapBytes - 1] = '\0';
    UnmapViewOfFile (buf);
    return STATUS_OK;
}

int Monitor::SendBitmapToOverlay (
    int visible, int width, int height, int stride, int seq, char *data, int dataSize)
{
    if ((this->mapFile == NULL) || (this->pid == 0))
    {
        Monitor::monitorLogger->error ("No process to draw overlay");
    ...
    if (!CheckTargetProcessAlive ())
    {
        Monitor::monitorLogger->error ("target process was terminated, dont send bitmap");
        this->pid = 0;
        this->processHandle = NULL;
        return TARGET_PROCESS_WAS_TERMINATED_ERROR;
    }
    if (width <= 0 || height <= 0 || stride < width * 4 || dataSize < stride * height ||
        width > static_cast<int> (GameOverlayIPC::OverlayFrameMaxWidth) ||
        height > static_cast<int> (GameOverlayIPC::OverlayFrameMaxHeight) || data == NULL)
    {
        Monitor::monitorLogger->error ("bad bitmap frame");
        return GENERAL_ERROR;
    }
    ...
    header->payloadSize = static_cast<std::uint32_t> (stride * height);
    CopyMemory ((PVOID)(buf + sizeof (GameOverlayIPC::OverlayFrameHeader)), data,
        header->payloadSize);
    UnmapViewOfFile (buf);
    return STATUS_OK;
}
